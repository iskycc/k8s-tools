...
- 依赖、插件和编译设置以 `pom.xml` 为准；当前使用 Apache MINA SSHD、Apache HttpClient 5（Java 8 下的 PATCH 支持）、Gson、JUnit 4。SLF4J NOP 为 optional，不强制传递给库的使用者。Bouncy Castle 仅为测试依赖。
