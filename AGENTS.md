...
- 当前源码的 Redis 接入优先使用 `K8sApiClient.builder().redisUrl(...).refreshCache(...).fromSsh(ssh[, options])`，连接池在初始化内创建并可靠关闭，返回的客户端无需 close；未配置 URL 不启用缓存，刷新必须有 URL。Builder 不修改传入 Options，自建缓存优先于旧 Options 外部缓存；原外部池不关闭。新 Builder 入口从 1.5.0 起提供，1.3.0 不包含。旧 `Options.redisCache(RedisServiceTokenCache)` 保留兼容，外部连接池由调用方管理。key 为 `host + ServiceToken`、`host + ApiServerUrl`，额外 `host + ServiceTokenMetadata` 保存配置归属与 CA，全部 String、无 TTL；MGET/MSET 保证同批读写，单实例 Redis，不支持 Cluster 跨槽。命中不连接 SSH；不完整、损坏或配置变化时重新获取。`invalidateCache()` 只删缓存，`refresh()` 先删除后 SSH 获取；不自动重放业务请求。Redis 失败必须显式抛错，不能静默绕过。
...
- `K8sApiClient.fromSsh`、新 `Builder.fromSsh` 和 CLI 默认直接跳过证书与主机名校验（含首次写入）；Builder 的 fromSsh 允许显式 TLS 设置，严格模式需同时关闭跳过和自动降级，不改变原 Builder.build 的默认值。CLI `--strict-tls` 显式关闭跳过和自动降级。Redis、静态 fromSsh 和 CLI 默认跳过 TLS 的行为从 1.3.0 起提供；Builder 托管 Redis 接入从 1.5.0 起提供。
...
- Redis 缓存变更：运行 `RedisCredentialsTest`、`ServiceTokenDiscoveryTest`、`MainDemoTest`，验证跨实例命中不执行 SSH、部分缓存/配置变化、401 后显式刷新、Redis 错误、连接归还及写操作不重放；客户端托管入口还需验证成功/异常后关闭 Redis socket、Options 不被修改、外部池仍可用以及严格 TLS。RESP mock 仅覆盖使用的命令，不代表完整 Redis 行为。
