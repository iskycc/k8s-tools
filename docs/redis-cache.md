...
Redis 缓存和静态 `K8sApiClient.fromSsh` 从正式版 `1.3.0` 起提供。本文的 **客户端托管 Redis** 入口 `K8sApiClient.builder().redisUrl(...).fromSsh(...)` 从正式版 `1.5.0` 起提供，使用 `io.github.iskycc:k8s-tools:1.6.2`；也可从源码 `mvn clean install` 后引用本地 `1.6.2-SNAPSHOT`。Jedis 固定为 `5.2.0`，使用方无需直接引用它的 API。
