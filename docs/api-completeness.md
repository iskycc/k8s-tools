...
新增 `K8sInstance` 七参数目标配置及 `K8sTools.init/refresh`，返回完整 K8sApiClient；Redis 密码内部编码、强制密码 SSH，复用已有凭据缓存和地址发现流程。初始化不保存全局集群，刷新返回新客户端且不重放请求。文档和 main 示例见[统一接入指南](k8s-instance.md)。

## 1.6.0 跨 namespace 资源搜索

`K8sApiClient` 新增 Pod、ConfigMap、Service 等 14 类资源的简易/详细搜索方法，以及适用于其他命名空间资源和 CRD 的通用入口。按名称字面子串匹配、自动遍历分页，返回全部匹配项；简易 DTO 仅保留 namespace/name（Pod 另带普通容器名称），详细结果保留常用元数据和完整 JSON。详见[搜索 SDK](resource-search.md)。搜索在客户端过滤，当前实现会先把列表全部页收集到内存；需要集群范围 list 权限，不把部分失败当成完整结果。新能力由模拟故障测试和真实集群跨 namespace 搜索测试覆盖，不引入新的依赖。
