...
| 正式版本 | `1.6.0`，新增跨 namespace 资源搜索的简易/详细 List 接口；保留 Pod Exec、SSH 回退与全局调试开关；包含客户端托管 Redis、完整查询 Demo、通用 CRUD、Discovery、分页和子资源接口 |
| 历史快照 | `1.1.0-SNAPSHOT`，需要额外仓库，不含 1.2.0 的 SSH 改动 |
| 当前源码开发版本 | `1.6.0-SNAPSHOT`，本次不发布该快照，可从源码安装到本地 |
| Java 包名 | `com.iskycc.k8s`，与 Maven groupId 不同 |
| 使用环境 | JDK 8+、Maven 3.6.3+ |
| 依赖类型 | 普通 jar，默认 `compile` scope，无需 classifier |

本次正式版坐标为 `1.6.0`，发布完成后可下载；需要试用快照时再切换版本与仓库。旧版 `1.0.0` 只有查询接口，不能编译本指南中的通用 CRUD 调用。
...
以下以历史快照 `1.1.0-SNAPSHOT` 为例，它不包含 `1.2.0` 的仅密码 SSH 模式。当前源码为 `1.6.0-SNAPSHOT`，本次只发布正式版，不上传新快照。使用历史快照时将依赖版本改为 `1.1.0-SNAPSHOT`，并在使用方 POM 的 `<project>` 下增加与 `<dependencies>` 同级的 `<repositories>`：
...
| 找不到 `resource`、`configMaps` 等方法 | 查看依赖树是否仍选中了 `1.0.0`；将坐标版本设为 `1.6.0` |
...
仓库当前 `pom.xml` 中的 `1.6.0-SNAPSHOT` 是源码开发构建版本，正式版坐标为 `1.6.0`。本指南不需要修改本仓库 POM 或运行任何发布命令。
