...
发布坐标为 `io.github.iskycc:k8s-tools`，许可证为 [Apache License 2.0](../LICENSE)。项目通过 Sonatype Central Portal 的 [Maven 发布插件](https://central.sonatype.org/publish/publish-portal-maven/) 上传，发布配置位于 [pom.xml](../pom.xml) 的 `release` profile。Maven `groupId` 与 Java 包名独立，Java API 的 import 仍使用 `com.iskycc.k8s`。
...
发布工作流只接受 `vX.Y.Z`，例如 `v1.0.0`。GitHub 预发布 Release 会被跳过；手动运行同样不接受 `SNAPSHOT`、`RC` 等后缀。单独推送标签不会触发自动发布，需发布 GitHub Release 或手动运行工作流。

每次部署包含主 jar、`-sources.jar`、`-javadoc.jar`、POM、各自产物的 `.asc` 签名，以及 Central 插件生成的校验和。工作流等待 Portal 确认发布完成；它不会创建 Git 提交、推送标签或修改分支上的 POM。
...
如果环境配置了部署标签限制，需要允许发布标签 `v*`。工作流的 `GITHUB_TOKEN` 由 GitHub 自动提供，只需要 `contents: read`，无需额外创建 GitHub PAT 或同名 Secret；Central 上传使用上面的 User Token。

## 发布一个版本

1. 将流水线、发布 POM 和许可证提交到仓库，确保目标代码通过 CI。
2. 确定版本，例如 `1.0.0`。对应标签中的 `pom.xml` 版本必须是 `1.0.0` 或 `1.0.0-SNAPSHOT`。发布下一个版本前，先将开发版本更新为相应的 `X.Y.Z-SNAPSHOT`。
3. 在 GitHub 创建并发布正式 Release，标签为 `v1.0.0`。标签必须指向包含发布配置的提交。
4. 工作流检出 `refs/tags/v1.0.0`，核对 POM 后，只在临时检出目录将版本设为 `1.0.0`，执行 `mvn -Prelease clean deploy`。
5. 在 Actions 查看结果，必要时到 Central Portal 的 Deployments 查看校验详情。发布后等待 Maven Central 同步，再验证消费者能解析 `io.github.iskycc:k8s-tools:1.0.0`。

需要手动运行时，在 Actions → Publish to Maven Central → Run workflow 中输入已有标签，如 `v1.0.0`。手动运行也会自动发布，并非仅构建预览。
...
mvn --batch-mode --no-transfer-progress \
  ...
  -DnewVersion=1.0.0 -DgenerateBackupPoms=false
